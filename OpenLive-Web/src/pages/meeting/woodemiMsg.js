import './meeting.scss';
import {APP_ID_LIVE as appId} from '../../utils/Settings';
...
const canvasHtml = '<canvas id="{{id}}" width="{{width}}" height="{{height}}" style="background: white;border:1px solid #d3d3d3;width:{{width}}px;height: {{height}}px";">';
...
$(() => {
  View.textarea = false;
  const accountName = Cookies.get('channel') || 'test';
  roomId = Cookies.get('channel') || new Date().getMilliseconds();
